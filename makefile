...
REPORTING_DIR := backend/services/reporting
...
.PHONY: help check-tools \
        setup sync \
        install-host install-auth install-notif install-catalog install-delivery install-reporting install-gateway install-frontend \
        up down restart build rebuild \
        ps logs \
        logs-auth logs-notif logs-catalog logs-delivery logs-reporting logs-gateway logs-frontend \
        logs-auth-db logs-notif-db logs-catalog-db logs-delivery-db logs-reporting-db \
        shell shell-auth shell-notif shell-catalog shell-delivery shell-reporting shell-gateway shell-frontend \
        psql-auth psql-notif psql-catalog psql-delivery psql-reporting \
        ...
        seed-admin seed-kpi seed-workbook \
...
help:
	...
	@echo "  make restart s=auth    Restart one service (s=auth|notification|catalog|delivery|reporting|gateway|frontend)"
	@echo ""
	@echo "Build"
	...
	@echo "  make logs-reporting    Tail reporting service logs"
	...
	@echo "  make logs-reporting-db Tail reporting_db logs"
	...
	@echo "  make shell-reporting   Shell into reporting container"
	...
	@echo "  make psql-reporting    psql into reporting_db"
	@echo ""
	...
	@echo "  make migrate-reporting                  Apply pending migrations to reporting"
	...
	@echo "  make test-reporting    Run reporting service tests"
...
migrate-create-reporting:
	@[ -n "$(m)" ] || { echo "Usage: make migrate-create-reporting m=\"message\""; exit 1; }
	$(COMPOSE) exec reporting alembic revision --autogenerate -m "$(m)"
...
migrate-current-reporting:
	@echo "── reporting ──"
	$(COMPOSE) exec reporting alembic current
...
migrate-history-reporting:
	@echo "── reporting ──"
	$(COMPOSE) exec reporting alembic history
...
lint:
	...
	cd $(REPORTING_DIR) && uv run ruff check .
...
format:
	...
	cd $(REPORTING_DIR) && uv run ruff format .
